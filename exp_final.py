"""
exp_final.py — combined approach for the core challenge.

Builds on the state-phase profile breakthrough (q1 gate = 0.01 at bottom,
0.29 near upright) by adding:
  - cos_q1 tracking instead of energy tracking
    → breaks spinning degeneracy (spinning ≠ upright in cos/sin)
  - small terminal anchor (W=0.1)
    → prevents end-of-trajectory drift back to bottom

If swing-up is achieved with default q_base_diag = [12, 5, 50, 40], this
demonstrates the full "core challenge": NN modulates Q-gates AND f_extra
to handle the QP's full default cost structure, with no hard zeroing.
"""
...
Q_BASE_DIAG = [12.0, 5.0, 50.0, 40.0]   # DEFAULT
GATE_RANGE_Q   = 0.99
GATE_RANGE_R   = 0.20
F_EXTRA_BOUND  = 3.0
F_KICKSTART    = 0.0
Q_GATE_KICKSTART_BIAS = -4.0   # q1 gate ≈ 0.011 init

# State-phase Q-profile (proven to enable state-dependent gates)
W_Q_PROFILE      = 100.0
Q_PROFILE_PUMP   = [0.01, 1.0, 1.0, 1.0]
Q_PROFILE_STABLE = [1.0,  1.0, 1.0, 1.0]
Q_PROFILE_STATE_PHASE = True

# cos_q1 tracking (breaks spinning degeneracy)
TRACK_MODE = "cos_q1"

# Small terminal anchor (prevents end-of-trajectory drift)
W_TERMINAL_ANCHOR = 0.2
...
def main():
    ...
    print("=" * 76)
    print("  FINAL EXP: state-phase profile + cos_q1 + terminal anchor")
    print(f"  q_base_diag = {Q_BASE_DIAG}  (DEFAULT)")
    print(f"  W_Q_PROFILE = {W_Q_PROFILE}  state_phase=True  W_TERM_ANCHOR={W_TERMINAL_ANCHOR}")
    print(f"  TRACK_MODE = {TRACK_MODE}  EPOCHS={EPOCHS}  LR={LR}")
    print("=" * 76)
    ...
    session_name = f"stageD_final_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    network_module.ModelManager(base_dir=SAVE_DIR).save_training_session(
        model=lin_net, loss_history=loss_history,
        training_params={
            "experiment": "final_state_phase_cos_q1",
