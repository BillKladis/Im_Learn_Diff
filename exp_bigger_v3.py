...
QF_DIAG     = [20.0, 50.0, 40.0, 30.0]   # qf50 brake
W_Q_PROFILE = 100.0
...
W_F_STABLE         = 50.0   # stab_state's recipe: state-conditional f_extra penalty
W_STABLE_PHASE     = 30.0
W_F_POS_ONLY       = 20.0   # position-only penalty (no velocity gating)   # state-pin in last 130 steps
...
def main():
    print("=" * 90)
    print(f"  EXP BIGGER NET (hidden=256) V3 (+ w_f_pos_only=20) — Qf q1d=50 + w_f_stable=50 + w_stable_phase=30")
    ...
    name = f"stageD_biggerv3_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
