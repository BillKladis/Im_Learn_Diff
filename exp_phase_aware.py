"""
exp_phase_aware.py — User's idea: phase-aware outer loss matching the
phase-aware Q-profile.

Outer loss structure:
  Pump phase (steps 0-118):  energy tracking (smooth gradient for pumping)
  Stable phase (steps 119+): position tracking (cos(q1)+1)² + sin(q1)²
                              + small velocity damping
                              → directly drives toward upright,
                                breaks spinning degeneracy

Q-profile target structure (state-phased, unchanged):
  pump   target = [0.01, 1, 1, 1]   (suppress q1 cost)
  stable target = [1.0,  1, 1, 1]   (full Q for stabilization)

Also, the demo trajectory has small residual error at the end
(q1=3.12 vs π=3.14).  We clean the last 20 steps to smoothly
interpolate to exactly [π, 0, 0, 0] so the stable-phase target is
clean.

Default q_base_diag = [12, 5, 50, 40].
"""
...
W_Q_PROFILE      = 100.0
# CRITICAL: suppress BOTH q1 and q1d during pump (matches ZERO_Q1_COSTS).
# With q1d gate at default 1.0, q1d_cost=5 damps q1_dot → kills pumping.
...
def clean_demo_tail(demo, num_smooth_steps=20):
    """Smoothly interpolate the last `num_smooth_steps` to exactly [π,0,0,0]."""
    ...
    for k in range(num_smooth_steps):
        # alpha goes from 0 (start of smoothing) to 1 (final state = goal)
        alpha = (k + 1) / num_smooth_steps
        # Smoothly blend, with cosine ease-in-out
...
def apply_q1_kickstart(lin_net, state_dim, horizon, bias_val):
    """Kickstart BOTH q1 and q1d dimensions to bias_val (gate ≈ 0.011)."""
...
def main():
    ...
    # Verify demo cleaning
    print(f"Demo last state (raw)    : {demo_raw[-1].cpu().numpy()}")
    print(f"Demo last state (cleaned): {demo[-1].cpu().numpy()}")
    print(f"Goal                     : {np.array(X_GOAL)}")

    print("\n" + "=" * 76)
    print("  EXP: phase-aware outer loss + state-phase Q-profile")
    print(f"  q_base_diag = {Q_BASE_DIAG}  (DEFAULT)")
    print(f"  Outer loss: pump=energy, stable=position+velocity damping")
    print(f"  Q-profile:  pump={Q_PROFILE_PUMP}, stable={Q_PROFILE_STABLE}")
    print(f"  Phase split frac = {PHASE_SPLIT_FRAC}  (steps 0-{int(PHASE_SPLIT_FRAC*NUM_STEPS)} pump)")
    print(f"  EPOCHS = {EPOCHS}  LR = {LR}  W_Q_PROFILE = {W_Q_PROFILE}")
    print("=" * 76)
    ...
    session_name = f"stageD_phaseaware_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    network_module.ModelManager(base_dir=SAVE_DIR).save_training_session(
        model=lin_net, loss_history=loss_history,
        training_params={
            "experiment": "phase_aware_outer_loss",
            "q_base_diag": Q_BASE_DIAG,
            "track_mode": TRACK_MODE,
            "w_q_profile": W_Q_PROFILE,
            "phase_split_frac": PHASE_SPLIT_FRAC,
            "demo_tail_cleaned": True,
        },
