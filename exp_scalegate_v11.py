"""exp_scalegate_v11.py — Soft f01 loss + lower structural guard (0.80) + larger perturbations.

KEY INSIGHT: The QPC tracking loss is a PROXY for f01. It mixes signals:
  - Top-start: "more gate = better tracking" ✓ (correct)
  - Bottom-start: "more gate = better tracking" ✗ (incorrect — gate slows arrival)
  The tracking loss can't distinguish "gate helps" from "gate hurts" for swing-up states.

SOFT F01 LOSS: Directly maximize mean(sigmoid(β × (0.10 - wrap_dist))).
  - Top-start: gradient = "gate = good near π" ✓ (wrap → 0 when gate ON)
  - Bottom-start: gradient = "gate at 127° = bad" ✓ (arrival delayed → fewer steps in zone)
  This naturally provides the iron-law gradient without manual phase-aware logic.

LOWER STRUCTURAL THRESHOLD (0.80 instead of 0.85):
  - Initial eval ~85% (genuinely sub-optimal, room to improve)
  - MLP can fire at near_pi ∈ [0.80, 0.85] zone
  - Soft f01 gradient at 0.80 zone during swing-up: NEGATIVE (delays arrival)
  - Soft f01 gradient at 0.80 zone during hold approach: POSITIVE (better hold)
  - MLP must LEARN to distinguish these (not structurally enforced)

LARGER HOLD PERTURBATIONS:
  - Top-start samples q1d up to ±1.5 rad/s (was ±0.6)
  - Targets the exact failure mode: near π but moderately fast → brief hold failures
  - Gives gradient signal for "gate during high-velocity approach to hold"

ARCHITECTURE: same as v10h (frozen lambda/mu, structural pos_gate, skip connection)
"""
...
NEAR_PI_SKIP_W    = 6.0
NEAR_PI_SKIP_B    = -5.0
STRUCTURAL_THRESH = 0.80   # LOWER than v10h (0.85) → genuinely sub-optimal start
STRUCTURAL_K      = 50.0
SOFT_F01_BETA     = 20.0   # smoothness of soft f01 gradient
WRAP_THRESH       = 0.10   # threshold matching the f01 metric


class GateVelMLP(nn.Module):
    """Same as v10h but with structural_thresh=0.80."""
...
def soft_f01_loss(x_traj, beta=SOFT_F01_BETA, wrap_thresh=WRAP_THRESH):
    """Negative soft f01 (minimize to maximize soft f01).

    x_traj: (T, 4) tensor of states [q1, q1d, q2, q2d]
    """
    angle_err = torch.atan2(
        torch.sin(x_traj[:, 0] - math.pi),
        torch.cos(x_traj[:, 0] - math.pi))
    dists = torch.sqrt(angle_err**2 + x_traj[:, 1]**2 +
                       x_traj[:, 2]**2 + x_traj[:, 3]**2)
    inside = torch.sigmoid(beta * (wrap_thresh - dists))
    return -inside.mean()
...
def sample_top_x0(device):
    """Larger perturbation than v10h (q1d up to ±1.5 instead of ±0.6)."""
    return torch.tensor([math.pi + (random.random()*2-1)*0.40,
                         (random.random()*2-1)*1.5,   # ← was 0.6, now 1.5
                         (random.random()*2-1)*0.30,
                         (random.random()*2-1)*0.8],  # ← was 0.6, now 0.8
...
def main():
    ...
    print("=" * 80)
    print(f"  EXP SCALEGATE v11: soft f01 loss + structural pos_gate({STRUCTURAL_THRESH}) + large perturbations")
    print(f"  Loss: -mean(sigmoid(β×(0.10 - wrap_dist)))  β={SOFT_F01_BETA}")
    print(f"  Direct optimization of f01 metric — bottom-start naturally penalizes swing-up gate")
    print(f"  struct_thresh={STRUCTURAL_THRESH} (sub-optimal start, MLP learns to sharpen gate)")
    print(f"  Top perturbation: q1d up to ±1.5 rad/s (targets hold-failure scenarios)")
    ...
    print(f"\n  Training ({args.top_frac:.0%} top / {1-args.top_frac:.0%} bottom, "
          f"NUM_STEPS={NUM_STEPS}, loss=soft_f01)...")
    ...
    while epoch < args.epochs and not interrupted[0]:
        ...
        x_traj, _ = train_module.rollout(
            lin_net=model, mpc=mpc, x0=x0_train, x_goal=x_goal, num_steps=NUM_STEPS)
        loss = soft_f01_loss(x_traj)
        loss.backward()
        # Clip gradients: soft f01 can have large gradients at wrap≈0.10 boundary
        ...
        if epoch % SAVE_EVERY == 0:
            f01_eval, arr, post = eval2k(model, mpc, x0_eval, x_goal)
            mark = ""
            if f01_eval > best_f01:
                mark = " ★"
                best_f01 = f01_eval
                best_state = {k: v.clone() for k, v in model.state_dict().items()
                              if not k.startswith('lin_net')}
            ...
            avg_train_f01 = float(np.mean(loss_history[-SAVE_EVERY:]))
            with torch.no_grad():
                ...
                x_140 = torch.tensor([math.radians(140), 0.0, 0.0, 0.0], dtype=torch.float64, device=device)
                ae_pi,  ar_pi,  _, _ = model._gate(x_pi)
                ae_mid, ar_mid, _, _ = model._gate(x_mid)
                ae_140, ar_140, _, _ = model._gate(x_140)
            print(f"  [ep={epoch:3d}]  {f01_eval:.1%}  arr={arr}  post={f'{post:.1%}' if post else 'N/A'}"
                  f"  k={k_eff:.4f}  α_eff@π={ae_pi:.4f}  α_eff@140°={ae_140:.4f}"
                  f"  α_eff@127°={ae_mid:.4f}  train_f01={avg_train_f01:.3f}"
