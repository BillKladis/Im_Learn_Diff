"""exp_boost_v2.py — Improved boost training: fresh x0 every epoch.

IMPROVEMENT OVER exp_boost_continue.py:
  boost_continue uses same x0 for 20 consecutive epochs (noisy!).
  This script samples a fresh near-top x0 EVERY epoch for diverse gradients.
  Also uses a cosine-annealed LR schedule for better convergence.

USAGE:
  python exp_boost_v2.py                    # starts from best checkpoint
  python exp_boost_v2.py --epochs 500       # more epochs
  python exp_boost_v2.py --lr 3e-3          # different LR
"""
...
def main():
    ...
    print("=" * 80)
    print(f"  EXP BOOST-V2: fresh x0 every epoch, cosine LR annealing")
    print(f"  Init dQ mean: {dQ_init.mean(0).tolist()}")
    print(f"  LR={args.lr}  NUM_STEPS={NUM_STEPS}  EPOCHS={args.epochs}")
    print(f"  x0 perturb: [π±{X0_PERT_Q1}, ±{X0_PERT_V1}, ±{X0_PERT_Q2}, ±{X0_PERT_V2}]")
    ...
    session_name = f"stageD_boost_v2_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    print(f"\n  Training (fresh x0 each epoch, cosine LR)...")
    epoch = 0
    losses = []
    while epoch < args.epochs and not interrupted[0]:
        # Fresh x0 every single epoch
