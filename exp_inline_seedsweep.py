"""exp_inline_seedsweep.py — multi-seed of the inline phase transition.
...
def train_one_seed(seed):
    ...
    name = f"stageD_seedsweep{seed}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    network_module.ModelManager(base_dir=SAVE_DIR).save_training_session(
        model=lin_net, loss_history=[],
        training_params={"experiment": "inline_seedsweep", "seed": seed,
                         "success": success},
        session_name=name,
    )
...
def main():
    print("=" * 78)
    print(f"  EXP INLINE SEEDSWEEP — {len(SEEDS)} seeds × ({PHASE1_EPOCHS}+{PHASE2_EPOCHS}) epochs")
    print(f"  Seeds: {SEEDS}")
    print("=" * 78)
