"""
exp_grad_split.py — Selective gradient training (the user's idea).

Detach gates_Q before passing to QP, so:
  - Energy-tracking loss → flows through f_extra (and trunk/encoder via
    f_head and r_head paths) → trains f_head to do pumping
  - Profile penalty → flows through gates_Q (without QP) → trains q_head
    to match state-dependent target (0.01 at bottom, 1.0 near upright)

The two gradient paths are CLEAN and don't interfere.  Q-head gets pure
supervised learning on the profile target.  f-head learns to pump given
whatever gates Q-head produces.

Default q_base_diag = [12, 5, 50, 40] — full default Q costs.
"""
...
def main():
    ...
    print("=" * 76)
    print("  EXP: GRADIENT SEPARATION — q_head trained by profile only,")
    print("                              f_head trained by tracking only")
    print(f"  q_base_diag = {Q_BASE_DIAG}  (DEFAULT)")
    print(f"  detach_gates_Q_for_qp = True")
    print(f"  W_Q_PROFILE = {W_Q_PROFILE}  state_phase = True")
    print(f"  TRACK = {TRACK_MODE}  EPOCHS = {EPOCHS}  LR = {LR}")
    print("=" * 76)
    ...
    loss_history, recorder = train_module.train_linearization_network(
        ...
        detach_gates_Q_for_qp=True,    # KEY CHANGE
    )
    ...
    print(f"  step={'q1':>8} {'q1d':>8} {'q2':>8} {'q2d':>8} {'fNorm':>8}  state(q1)")
    ...
    session_name = f"stageD_gradsplit_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    network_module.ModelManager(base_dir=SAVE_DIR).save_training_session(
        model=lin_net, loss_history=loss_history,
        training_params={
            "experiment": "gradient_separation",
