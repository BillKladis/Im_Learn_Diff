"""exp_scalegate_v10h.py — MLP gate with structural pos_gate + frozen lambda/mu.

FAILURE CHAIN v10g (ep=30: 0.0%):
  v10g had structural pos_gate (0.85) preventing gate expansion into swing-up zone.
  This worked: α_eff@127°=0.073 stayed small (structural guarantee held).
  Training progress: init=87.2%, ep=10=87.4%★, ep=20=87.5%★, ep=30=0.0%.

  But catastrophic collapse at ep=30: arr=238 (pendulum DID arrive) but post=0.0%.
  Root cause: lambda_head and mu_head also have kaiming trunk feeding non-zero h.
    lambda_head.weight was zeroed (lam starts at ones) BUT gets gradient from loss.
    By ep=30, lambda_head learned to amplify dQ_ref per-dimension (lam ≠ ones).
    If lam learned to amplify Q[q1] by e.g. 5×, the MPC over-corrects and oscillates.
    Or mu_head reduced R → larger control actions → instability.
    The nonlinear jump from 87.5% → 0.0% in 10 epochs is consistent with
    a parameter crossing a critical stability boundary in the QPC landscape.

FIX: FREEZE lambda_head and mu_head.
  Remove them from the optimizer → weights stay at 0, bias stays at 1 → lam=ones, mu=ones always.
  Only train: trunk, alpha_head, gate_k_raw.

  This focuses learning entirely on alpha (WHEN to apply the gate and at what scale).
  The dQ_ref direction is already optimal (from scale4 checkpoint). lambda/mu reweighting
  adds risk without clear benefit.

WHAT THE MLP LEARNS (trunk + alpha only):
  1. Push α_raw@π from 0.731 → 1.0 via trunk positive contribution near π
  2. Structural pos_gate prevents expansion below near_pi=0.85 (structural guarantee)
  3. vel_gate (learnable k) adjusts velocity threshold
  4. No lambda/mu interference with dQ_ref direction

EXPECTED: Init=87.2%, training pushes toward 87.5-87.9% as α_raw@π → 1.0.
"""
...
class GateVelMLP(nn.Module):
    """MLP gate with structural pos_gate + vel_gate. Lambda/mu FROZEN at identity.

    alpha_eff = sigmoid(alpha_head([trunk, near_pi])) × pos_gate × vel_gate
    gQ += alpha_eff × dQ_ref   (lam=ones, frozen)
    gR += alpha_eff × dR_ref   (mu=ones, frozen)
    fe *= (1 - alpha_eff)
    """
    def __init__(self, lin_net, dQ_ref, dR_ref, hidden=16):
        ...
        # Trunk: kaiming default (active — non-zero h → gradient flows to alpha_head.W_trunk)
        # alpha_head: trunk part=0 (clean start), skip weight=SKIP_W, bias=SKIP_B
    ...
    def _gate(self, x_last):
        """Returns (alpha_eff, alpha_raw, vel_gate, pos_gate)."""
    ...
    def forward(self, x_sequence, q_base_diag=None, r_base_diag=None):
        ...
        gQ = gQ + alpha * self.dQ_ref       # lam=ones (frozen)
        gR = gR + alpha * self.dR_ref       # mu=ones (frozen)
        fe = fe * (1.0 - alpha)
...
def main():
    ...
    # Only train trunk + alpha_head + gate_k_raw. Lambda/mu are FROZEN (implicit lam=ones, mu=ones).
    trainable = (list(model.trunk.parameters()) +
                 list(model.alpha_head.parameters()) +
                 [model.gate_k_raw])
    n_trainable = sum(p.numel() for p in trainable)
    n_total = n_trainable  # no lambda/mu params in model

    k_eff_init = F.softplus(torch.tensor(GATE_K_INIT)).item()
    vel_thresh_init = (1.0 / k_eff_init) ** 0.5
    print("=" * 80)
    print(f"  EXP SCALEGATE v10h: MLP alpha-only + structural pos_gate(0.85) + vel_gate")
    print(f"  FIX: lambda/mu FROZEN (lam=ones, mu=ones) → no dQ_ref direction distortion")
    ...
    print(f"  Trunk: 4→{args.hidden}→{args.hidden}  |  {n_trainable} trainable params")
    ...
    while epoch < args.epochs and not interrupted[0]:
        ...
        if epoch % SAVE_EVERY == 0:
            ...
            with torch.no_grad():
                ...
                ae_pi,  ar_pi,  _, pg_pi  = model._gate(x_pi)
                ae_mid, ar_mid, _, pg_mid = model._gate(x_mid)
    ...
    session_name = f"stageE_scalegate_v10h_h{args.hidden}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    network_module.ModelManager(base_dir="saved_models").save_training_session(
        model=lin_net, loss_history=[],
        training_params={"experiment": "scalegate_v10h", "best_f01": best_f01,
