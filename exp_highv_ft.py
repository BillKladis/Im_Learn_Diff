"""exp_highv_ft.py — High-velocity-damping MPC fine-tuning.

ROOT CAUSE ANALYSIS:
  The pendulum oscillates in the wrap 0.1-0.3 range and won't settle.
  Diagnosis: gates_Q ∈ [0.05, 1.95] (gate_range_q=0.95), so with
  q_base_diag[1]=5, the MAX q1d velocity weight = 5 × 1.95 = 9.75.
  This is too weak to damp the top-link oscillation.

FIX:
  Change q_base_diag from [12, 5, 50, 40] to [12, 20, 50, 60]:
    Near upright (gates_Q≈[1,1,1,1]):
      Old effective Q: [24, 10, 100, 80]
      New effective Q: [24, 40, 100, 120]  ← 4× stronger q1d damping!

  The profile target q_profile_stable=[1,1,1,1] now drives the QP to use
  much stronger velocity damping near the upright without any network
  architecture change.

  Trade-off: during swing-up, q1d weight also increases (5→20). This is
  partially compensated by q_profile_pump=[0.01,0.01,1,1] driving gates
  toward 0.01, but not fully. f_extra (fNorm≈4) must still provide enough
  energy pumping.

Starting point: posonly_ft ep75 (frac<0.10=3.8%, fNorm=4.0).
"""
...
DT           = 0.05
EPOCHS       = 90
LR           = 3e-5        # conservative for targeted tuning
HORIZON      = 10
SAVE_DIR     = "saved_models"

# KEY CHANGE: higher velocity weights for stronger top-link damping
Q_BASE_DIAG  = [12.0, 20.0, 50.0, 60.0]   # was [12, 5, 50, 40]
# Effective Q at upright (gates=1):    [24, 40, 100, 120]
# Effective Q at upright (gates=0.01): [12.12, 20.2, 100.5, 60.6]
# Compare old effective Q (gates=1):   [24, 10, 100, 80]

W_F_POS_ONLY     = 50.0    # same as proven posonly_ft
W_Q_PROFILE      = 100.0
Q_PROFILE_PUMP   = [0.01, 0.01, 1.0, 1.0]
Q_PROFILE_STABLE = [1.0,  1.0,  1.0, 1.0]  # gates→1 near top → effective Q=[24,40,100,120]
W_END_Q_HIGH     = 0.0     # removed: profile already handles gate targets
END_PHASE_STEPS  = 20
SAVE_EVERY       = 15
...
def _find_pretrained():
    ...
    if ckpts:
        return ckpts[-1], f"posonly_ep{os.path.basename(os.path.dirname(ckpts[-1]))[-3:]}"
...
def main():
    ...
    print("=" * 80)
    print("  EXP HIGHV-FT: high-velocity-damping Q fine-tuning")
    print(f"  Pretrained: {src_label}")
    print(f"  Q_BASE_DIAG={Q_BASE_DIAG}  (was [12,5,50,40])")
    print(f"  Effective Q at upright (gates=1): {[2*q for q in Q_BASE_DIAG]}")
    print(f"  w_f_pos_only={W_F_POS_ONLY}  LR={LR}  EPOCHS={EPOCHS}")
    print("=" * 80)
    ...
    session_name = f"stageD_highv_ft_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

    print("\n  Pre-eval (with new Q_BASE_DIAG):")
