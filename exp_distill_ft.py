"""exp_distill_ft.py — Distillation-only f_head fine-tuning (no tight_top).

WHY TIGHT_TOP ALWAYS FAILS:
  w_f_tight_top fires during the TRAINING TRAJECTORY whenever near_pi > thresh.
  This conflicts with the energy tracking loss (which needs f_extra high for
  swing-up). Gradient descent compromises: reduces f_extra globally.
  Frozen trunk/encoder doesn't help — f_head weights still see conflicting
  gradients from tight_top (during trajectory when near top) vs energy tracking.

KEY INSIGHT: Use w_distill_goal instead.
  Distillation trains f_extra→0 at SYNTHETIC near-goal states (not on trajectory).
  No conflict with energy tracking: synthetic states are separate from rollout.
  f_head must learn: "when trunk features say near_pi > 0.8, output 0."
  Trunk is FROZEN so features are consistent. f_head CAN learn this.

DESIGN:
  - Freeze: state_encoder, trunk, q_head, r_head (ALL except f_head)
  - Train: f_head only (LR=3e-4)
  - w_distill_goal=3000: strong signal to drive f_extra→0 near goal
  - distill_pos_pert=1.0: covers |q1-π| < 1.0 rad (near_pi > 0.54) — wide
  - distill_vel_pert=2.0, distill_q2_pert=2.0: cover dynamic hold states
  - distill_n_synth=64: good coverage of near-top state space
  - w_f_phase_reward=200: strong reward for f_extra during swing-up
  - NO w_f_tight_top: eliminates trajectory conflict entirely

ZeroFNet target: 25.9% frac<0.10 (2000-step), 31.0% post-arrival.
"""
...
DT           = 0.05
EPOCHS       = 120
LR           = 3e-4       # higher: only f_head (35k params) updates
HORIZON      = 10
SAVE_DIR     = "saved_models"
Q_BASE_DIAG  = [12.0, 5.0, 50.0, 40.0]

# DISTILLATION — no trajectory conflict
W_DISTILL_GOAL   = 3000.0  # strong: drive f_extra→0 at synthetic near-goal states
DISTILL_N_SYNTH  = 64      # good coverage of near-top state space
DISTILL_POS_PERT = 1.0     # covers |q1-π| < 1.0 rad (near_pi > 0.54)
DISTILL_VEL_PERT = 2.0     # large velocity range for dynamic hold states
DISTILL_Q2_PERT  = 2.0     # large q2 range

# NO tight_top: eliminates trajectory conflict entirely
W_F_TIGHT_TOP    = 0.0

# Phase reward: keep f_extra high during swing-up (prevents f_head global collapse)
W_F_PHASE_REWARD = 200.0   # strong counterbalance to distillation signal

W_F_POS_ONLY  = 0.0   # distill handles near-top
...
def main():
    ...
    print("=" * 80)
    print("  EXP DISTILL-FT: distillation into f_head (no tight_top conflict)")
    print(f"  Pretrained: {src_label}")
    print(f"  FROZEN: state_encoder, trunk, q_head, r_head")
    print(f"  TRAINED: f_head only (LR={LR})")
    print(f"  w_distill_goal={W_DISTILL_GOAL}  n_synth={DISTILL_N_SYNTH}")
    print(f"  pos_pert={DISTILL_POS_PERT}  vel_pert={DISTILL_VEL_PERT}  q2_pert={DISTILL_Q2_PERT}")
    print(f"  w_f_phase_reward={W_F_PHASE_REWARD}  (counteracts distill collapse during swing-up)")
    ...
    session_name = f"stageD_distill_ft_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    while chunk_start < EPOCHS and not interrupted[0]:
        ...
        loss_chunk, recorder = train_module.train_linearization_network(
            ...
            distill_n_synth=DISTILL_N_SYNTH,
            distill_pos_pert=DISTILL_POS_PERT,
            distill_vel_pert=DISTILL_VEL_PERT,
            distill_q2_pert=DISTILL_Q2_PERT,
