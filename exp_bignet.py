"""exp_bignet.py — Train from scratch with hidden_dim=256 (2× capacity).

Hypothesis: the 128-dim network may not have enough capacity to represent
multiple swing-up strategies simultaneously (different initial conditions,
both CW and CCW directions).  A 256-dim network trained from scratch might
naturally learn a more general policy.

Identical training setup to exp_no_demo.py (the best result):
  - Synthetic cosine-eased energy ramp as target
  - State-phase Q-profile (pump→stable blend based on cos(q1-π))
  - End-phase Q-gate increase for stabilisation
  - q1+q1d kickstart bias = -3

The only change: hidden_dim=256 instead of 128.
"""
...
LR        = 1e-3
HORIZON   = 10
HIDDEN_DIM = 256      # <-- the only change vs exp_no_demo.py
...
def main():
    ...
    print("=" * 76)
    print(f"  EXP: BIGGER NETWORK — hidden_dim={HIDDEN_DIM} (vs 128 baseline)")
    print(f"  Same training setup as exp_no_demo.py (best result: 0.0612)")
    ...
    n_params_128 = sum(p.numel() for p in network_module.LinearizationNetwork(
        state_dim=STATE_DIM, control_dim=CONTROL_DIM,
        horizon=HORIZON, hidden_dim=128,
    ).parameters())
    ...
    n_params_256 = sum(p.numel() for p in lin_net.parameters())
    print(f"  Network params: {n_params_256:,} (vs {n_params_128:,} for 128-dim)")
    ...
    session_name = f"stageD_bignet_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    network_module.ModelManager(base_dir=SAVE_DIR).save_training_session(
        model=lin_net, loss_history=loss_history,
        training_params={
            "experiment": "bignet_256dim",
