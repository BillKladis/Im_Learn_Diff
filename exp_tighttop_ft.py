"""exp_tighttop_ft.py — Tight near-top f_extra penalty for hold quality.

KEY DISCOVERY:
  Zeroing f_extra when near_pi > 0.9 (within 37° of upright) gives:
    frac<0.10 = 25.9%  (vs 5.2% baseline posonly_ft ep75)
    post-arrival<0.10 = 31.0%  (vs 6.2% baseline)
  This 5× improvement is achieved WITHOUT retraining.

WHY THIS WORKS:
  The pendulum oscillates in the 0.1-0.3 zone because f_extra is pumping
  energy at the TOP of each oscillation (high-velocity swing-through).
  Zeroing f_extra when near_pi>0.9 stops this energy injection at the top,
  reducing oscillation amplitude dramatically.

  Velocity-gating (near_pi>0.9 AND low velocity) is WORSE because it only
  suppresses f_extra during the hold phase but not during swing-through, so
  the oscillation amplitude stays high.

TRAINING GOAL:
  Train the network to natively output f_extra≈0 when near_pi > 0.8.
  Use w_f_tight_top (new in Simulate.py): linear ramp from 0 at tight_top_thresh
  to full strength at near_pi=1.0. Completely absent below tight_top_thresh,
  so swing-up energy building is unaffected.

  tight_top_thresh=0.8 → fires for |q1-π| < 53° (near the top)

Starting from: posonly_ft ep75 (frac<0.10=3.8%, fNorm=4.0, 6.3% post-arrival).
"""
...
# KEY: tight near-top penalty — only fires near the upright (no swing-up harm)
W_F_TIGHT_TOP    = 500.0   # strong penalty to drive f_extra→0 near top
TIGHT_TOP_THRESH = 0.8     # fires when near_pi > 0.8 (|q1-π| < 53°)

# Keep mild broad penalty too (posonly_ft recipe)
W_F_POS_ONLY     = 10.0    # lighter than posonly_ft's 50 (tight_top handles near-top)
...
def main():
    ...
    print("=" * 80)
    print("  EXP TIGHTTOP-FT: tight near-top f_extra penalty")
    print(f"  Pretrained: {src_label}")
    print(f"  w_f_tight_top={W_F_TIGHT_TOP}  thresh={TIGHT_TOP_THRESH}  w_f_pos_only={W_F_POS_ONLY}")
    print(f"  Fires when near_pi > {TIGHT_TOP_THRESH} (|q1-π| < {math.degrees(math.acos(2*TIGHT_TOP_THRESH-1)):.1f}°)")
    print(f"  ZeroFNet test showed: 25.9% frac<0.10 at near_pi>0.9 threshold")
    print("=" * 80)
    ...
    session_name = f"stageD_tighttop_ft_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
