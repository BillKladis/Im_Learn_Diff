"""exp_real_hold.py — train for ACTUAL hold (not just passing through goal).

Diagnosis: qf50 v2 has 12/12 'generality' but NEVER actually settles
at upright. Wrap distance oscillates 0.3 → 6.7 → 0.3 → 6.7 forever.
The pendulum continuously swings through.

Fix: heavy STATE-MODE position pin in the last N steps. This forces
the network to find a policy that actually arrives AND stays — not
just brushes the goal.

Recipe (all in one shot, no phases, no fine-tune):
  - 0.0612 swing-up loss (track-energy + q_profile + end_q_high)
  - Qf q1d=50 (the brake)
  - w_stable_phase=30 in last 130 steps (pins wrapped(q1-π)² + q1d² + q2² + q2d²)
  - NUM_STEPS=300 (170 swing-up + 130 hold)
  - Network can't 'cheat' by swinging through the goal —
    every swing-through during the last 130 steps gets penalized.
"""
...
# THE NEW INGREDIENT — state-pin loss in last 130 steps
W_STABLE_PHASE   = 30.0
...
def make_demo(num_steps, swingup_steps, device):
    """Cosine-eased q1 ramp 0 → π for first 170 steps, then hold at π."""
    ...
    def log_epoch(self, epoch, num_epochs, loss, info):
        ...
        if self._call % 5 == 0 or self._call == 1:
            ...
            print(f"  ep {self._call:>3}  loss={loss:>9.3f}  track={info.get('loss_track',0):.3f}  "
                  ...
                  f"arr={arr_str:>5}  long={lng:>4}  tot={tot:>4}  tight={tight:.1%}  "
...
def main():
    print("=" * 90)
    print(f"  EXP REAL HOLD — single-shot from scratch with state-pin in last {STABLE_PHASE_STEPS} steps")
    print(f"  Recipe: 0.0612 swing-up + Qf q1d=50 + w_stable_phase={W_STABLE_PHASE}")
    ...
    monitor = M(mpc, lin_net, x0, x_goal)
    ...
    print(f"  Best long={monitor._best_long}  best_tight(<0.1)={monitor._best_tight:.1%}")
    lin_net.load_state_dict(monitor._best_state)

    name = f"stageD_realhold_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    network_module.ModelManager(base_dir=SAVE_DIR).save_training_session(
        model=lin_net, loss_history=[],
        training_params={"experiment": "real_hold_state_pin",
                         "qf_diag": QF_DIAG,
                         "w_stable_phase": W_STABLE_PHASE,
    ...
    print(f"\n  Final eval (2000 steps):")
    for thr in [0.05, 0.1, 0.2, 0.3, 0.5, 1.0]:
        print(f"    fraction wrap<{thr:.2f}: {(wraps<thr).mean():.1%}")
    arr, lng, tot, tight = hold_metric(traj, np.array(X_GOAL))
    print(f"  arrival: {arr}  longest contiguous (<0.3): {lng}  total: {tot}")
