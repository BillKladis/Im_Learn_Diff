"""exp_no_demo_realistic.py — original recipe + REAL trajectory as demo
+ heavy late-phase track penalty.

User direction: 'lets explore a more consistent energy structure with
realistic shape and numbers' AND 'maybe heavily penalize deviation
from the target after it is approaching the target to encourage
higher q and qf'.

Two coupled changes vs the 0.0612-baseline recipe:

  1. DEMO: load the actual stab_state model's reference trajectory
     and use the first 170 states as the demo. The demo's energy
     curve now has the realistic shape we measured:
        E ramps -14.7 → +14.77 (slight overshoot, peak at step 164)
        K (kinetic) peaks at step 140 (NOT mid-trajectory)
        E settles to ≈ 14.7 right after arrival

  2. LATE-PHASE PRESSURE: w_track_late_phase=10 in last 50 steps.
     The track loss is normally averaged over all 170 steps; with
     this on, the last 50 steps each get a 10x multiplier. The
     network is forced to track the late energy/state target very
     tightly — which can only be achieved by raising Q (and the
     existing Qf via base config). 'Higher q and qf' that the user
     wants comes from the network maximising the QP's track-keeping.

ONLY changes vs exp_no_demo. Qf default. No qf_head. No curriculum.
Single-shot test with realistic data + heavy late penalty.
"""

import math, os, sys, time, csv
...
REF_CSV = "saved_models/stageD_trajcurr_20260429_075636/reference_trajectory.csv"
...
def load_reference_demo(csv_path, num_steps, device):
    """Load first num_steps states from a successful rollout's CSV."""
    states = []
    with open(csv_path) as f:
        for row in csv.DictReader(f):
            ...
            if len(states) >= num_steps:
                break
    if len(states) < num_steps:
        raise ValueError(f"CSV has only {len(states)} rows, need {num_steps}")
    return torch.tensor(states, dtype=torch.float64, device=device)
...
def main():
    ...
    # Verify demo's energy at key steps
    print("\n  Demo (real trajectory) energy at key steps:")
    for idx in [0, 30, 60, 90, 120, 140, 150, 160, 165, 169]:
        e = mpc.compute_energy_single(demo[idx]).item()
        print(f"    step {idx:>3}: q1={demo[idx,0].item():.3f}  "
              f"q1d={demo[idx,1].item():.3f}  E={e:.3f}")
    ...
    session_name = f"stageD_realistic_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    network_module.ModelManager(base_dir=SAVE_DIR).save_training_session(
        model=lin_net, loss_history=loss_history,
        training_params={
            "experiment": "no_demo_realistic_late_phase",
