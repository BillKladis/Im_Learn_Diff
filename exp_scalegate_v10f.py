"""exp_scalegate_v10f.py — Learned gate with active trunk + skip + structural vel suppressor.

LESSONS FROM v10e FAILURE (ep=30: f01 stuck at 82.9%, α@π declining):
  Root cause: dead trunk — all trunk weights zeroed → trunk output = 0 always.

  With trunk_output = 0:
    d(loss)/d(alpha_head.W_trunk[j]) = d(loss)/d(alpha_out) × h2[j] = grad × 0 = 0
    d(loss)/d(trunk_weights) = ... × alpha_head.W_trunk × h2 = ... × 0 × 0 = 0
  → Both trunk AND alpha_head.W_trunk get ZERO gradient → trunk never wakes up.

  The only thing learning was W_skip and bias via the near_pi skip.
  W_skip converged to ≈5.9 (gradient equilibrium, top/bottom cancel).
  α@π stuck at 0.71, threshold sharpened to 0.851 but sigmoid still too gradual.
  A sigmoid with W=5.9 can only reach 0.731 at π — nowhere near 1.0.
  For 87.3%, we need W→∞ (sharp step) or the trunk to actively sharpen the gate.

FIX: Use DEFAULT (kaiming) trunk init so trunk outputs non-zero h.
  With h ≠ 0: d(loss)/d(alpha_head.W_trunk[j]) = grad × h[j] ≠ 0 → starts learning.
  Then as alpha_head.W_trunk grows: trunk itself gets non-zero gradient → bootstraps.

  alpha_head.W_trunk STILL starts at 0 (so initial alpha controlled by skip only).
  Trunk weights random (kaiming_uniform) → h ≠ 0 → gradient flows.
  Result: initial gate profile = sigmoid(6×near_pi-5) as before (trunk contribution=0
  initially), but trunk + alpha_head.W_trunk can learn to SHARPEN the gate.

ARCHITECTURE (same as v10e, only trunk init changed):
  trunk: kaiming init (default) — NOT zeroed
  alpha_head.W_trunk: zeros init (so initial alpha is just sigmoid(skip))
  alpha_head.W_skip = 6.0, alpha_head.bias = -5.0 (same as v10e)
  All else unchanged.
"""
...
class GateVelMLP(nn.Module):
    ...
    def __init__(self, lin_net, dQ_ref, dR_ref, hidden=16):
        ...
        # Trunk: DEFAULT kaiming init (non-zero → gradients flow through alpha_head.W_trunk)
        # Do NOT zero the trunk — zero trunk → zero gradient → dead trunk → trunk never learns.
...
def main():
    ...
    print("=" * 80)
    print(f"  EXP SCALEGATE v10f: MLP gate + skip connection + structural vel suppressor")
    print(f"  FIX: active trunk (kaiming) → gradients flow → trunk learns to sharpen gate")
    ...
    session_name = f"stageE_scalegate_v10f_h{args.hidden}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    network_module.ModelManager(base_dir="saved_models").save_training_session(
        model=lin_net, loss_history=[],
        training_params={"experiment": "scalegate_v10f", "best_f01": best_f01,
