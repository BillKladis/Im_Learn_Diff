"""exp_scalegate_v10d.py — Learned gate with structural velocity suppressor.

LESSONS FROM v10c FAILURE (ep=10: still 5.1%, same as init):
  Root cause: dead gradient at clamp boundary.
    alpha_head weight=0, bias=0 → output=0 exactly → clamp(0,1) at lower boundary
    PyTorch clamp gradient = 0 when input ≤ 0 → no gradient flows → no learning.
    k_eff drifted 0.3133→0.3146 only from AdamW weight decay, not loss signal.

FIX:
  Replace clamp(0,1) with sigmoid() for alpha_raw.
  Initialize alpha_head.bias = -3.0 → sigmoid(-3) ≈ 0.047 (small but non-zero).
  Gradient of sigmoid at x=-3: σ(-3)(1-σ(-3)) ≈ 0.045. Non-zero → learning happens.
  LR raised to 0.01 (safe: velocity suppressor blocks high-velocity catastrophe).

DESIGN (unchanged from v10c except activation + bias + LR):
  Same architecture: GateVelMLP with structural velocity suppressor.

    k_eff = softplus(gate_k_raw)  [global scalar, ≥0, init ≈ 0.313]
    vel_factor = clamp(1 - k_eff × q1d², 0, 1)
    alpha_raw = sigmoid(alpha_head(trunk(features)))  ← CHANGED from clamp
    alpha_effective = alpha_raw × vel_factor

  alpha_head.bias init = -3.0 → alpha_raw ≈ 0.047 uniform at start (4.7%).
  Structural velocity suppressor still blocks gate at q1d≥1.79 rad/s.
  NUM_STEPS=300 (> arr=242) so bottom-start rollouts see BOTH swing-up AND hold.
  LR=0.01, top_frac=0.5, 400 epochs.
"""
...
ALPHA_BIAS_INIT = -3.0  # → sigmoid(-3) ≈ 0.047 uniform at start (non-zero gradient!)


class GateVelMLP(nn.Module):
    """MLP gate with structural velocity suppressor.

    alpha_effective = sigmoid(alpha_head(trunk)) × clamp(1 - k_eff × q1d², 0, 1)
    ...
    def __init__(self, lin_net, dQ_ref, dR_ref, hidden=16):
        ...
        # alpha: sigmoid activation with negative bias → small initial value, non-zero gradient
        nn.init.zeros_(self.alpha_head.weight)
        nn.init.constant_(self.alpha_head.bias, ALPHA_BIAS_INIT)
    ...
    def _gate(self, x_last):
        ...
        alpha_raw  = torch.sigmoid(self.alpha_head(h).squeeze())  # sigmoid, not clamp
...
def main():
    ...
    alpha_init = torch.sigmoid(torch.tensor(ALPHA_BIAS_INIT)).item()
    print("=" * 80)
    print(f"  EXP SCALEGATE v10d: MLP gate + structural velocity suppressor")
    print(f"  FIX: sigmoid(alpha_head) instead of clamp — dead gradient bug fixed")
    print(f"  alpha_eff = sigmoid(alpha_head(trunk)) × clamp(1 - k_eff×q1d², 0, 1)")
    print(f"  k_eff = softplus({GATE_K_INIT}) ≈ {k_eff_init:.3f} → gate off at q1d≥{vel_thresh_init:.2f}")
    print(f"  alpha_head.bias init={ALPHA_BIAS_INIT} → sigmoid({ALPHA_BIAS_INIT})≈{alpha_init:.4f} (non-zero gradient)")
    ...
    while epoch < args.epochs and not interrupted[0]:
        ...
        if epoch % SAVE_EVERY == 0:
            ...
            alpha_q180 = torch.sigmoid(model.alpha_head(
                model.trunk(model._features(
                    torch.tensor([math.pi, 0.0, 0.0, 0.0], dtype=torch.float64)
                ).unsqueeze(0))
            ).squeeze()).item()
            print(f"  [ep={epoch:3d}]  {f01:.1%}  arr={arr}  post={f'{post:.1%}' if post else 'N/A'}"
                  f"  k={k_eff:.4f}  α@π={alpha_q180:.4f}  top/bot={top_count}/{bot_count}"
    ...
    session_name = f"stageE_scalegate_v10d_h{args.hidden}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    network_module.ModelManager(base_dir="saved_models").save_training_session(
        model=lin_net, loss_history=[],
        training_params={"experiment": "scalegate_v10d", "best_f01": best_f01,
