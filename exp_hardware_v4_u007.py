"""exp_hardware_v4_u007.py — Fine-tune hw_v1_ep50 with u_max=0.07 Nm torque limit.

Aggressive constraint: 47% of the original 0.15 Nm authority. This tests
whether the double pendulum can still achieve reliable swing-up and hold
under very tight torque limits. The physical gravity torque at q1=π/2 is
~0.089 Nm, so direct swing-up is not feasible — energy pumping is required
over multiple swings.

Target: f01≥70% clean with u_max=0.07 Nm (may need more epochs to converge).
"""
...
DT     = 0.05
HORIZON = 10
U_LIM   = 0.07   # Nm — very tight, 47% of default 0.15 Nm
...
META_EPOCHS      = 200   # more epochs — energy pumping convergence is slower
N_BOTTOM_PER_TOP = 4     # more bottom passes to learn pumping
N_BOTTOM         = 25
N_TOP            = 100
LR               = 3e-4   # slightly lower LR — tighter constraint needs gentler steps
WEIGHT_DECAY     = 1e-4
...
TOP_PERT_Q1  = 0.20   # tighter perturbations — less authority to recover
TOP_PERT_Q1D = 0.20
TOP_PERT_Q2  = 0.15
TOP_PERT_Q2D = 0.20
...
def main():
    ...
    out("=" * 80)
    out("  EXP: HARDWARE v4 — u_max=0.07 Nm fine-tune from hw_v1_ep50")
    out(f"  device: {device}")
    out(f"  U_LIM={U_LIM}  LR={LR}  META_EPOCHS={META_EPOCHS}")
    out("  NOTE: gravity at q1=π/2 is ~0.089 Nm > u_max. Energy pumping required.")
    out("=" * 80)
    ...
    out(f"\n  Starting fine-tuning with u_max={U_LIM} Nm ...\n")
