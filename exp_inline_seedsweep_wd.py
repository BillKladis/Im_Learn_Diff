"""exp_inline_seedsweep_wd.py — multi-seed of the inline phase transition.
...
def train_one_seed(seed):
    ...
    optimizer = torch.optim.AdamW(lin_net.parameters(), lr=LR, weight_decay=1e-2)
    ...
    name = f"stageD_seedsweepWD{seed}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    network_module.ModelManager(base_dir=SAVE_DIR).save_training_session(
        model=lin_net, loss_history=[],
        training_params={"experiment": "inline_seedsweep_wd", "seed": seed,
                         "success": success},
        session_name=name,
    )
...
def main():
    print("=" * 78)
    print(f"  EXP INLINE SEEDSWEEP (high weight decay) — {len(SEEDS)} seeds × ({PHASE1_EPOCHS}+{PHASE2_EPOCHS}) epochs")
    print(f"  Seeds: {SEEDS}")
    print("=" * 78)
